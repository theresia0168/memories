...
-- Schema memories
-- -----------------------------------------------------
-- 코리아IT아카데미 부산 Memories 프로젝트 데이터베이스
-- 
...
CREATE TABLE IF NOT EXISTS `memories`.`user` (
  ...
  `sns_id` VARCHAR(100) NULL COMMENT 'OAuth 인증 후 인증 서버로부터 발급한 ID',
...
CREATE TABLE IF NOT EXISTS `memories`.`memory_test` (
  `user_id` VARCHAR(20) NOT NULL COMMENT '검사를 수행한 사용자의 ID',
  `sequence` INT NOT NULL COMMENT '사용자별 검사 순번\n',
  `measurement_time` INT NOT NULL COMMENT '측정된 시간(검사 결과)\n',
  `test_date` VARCHAR(16) NOT NULL COMMENT '검사한 일자',
  `gap` INT NULL COMMENT '(직전 순번의 측정된 순간) - (현재 순번의 측정된 시간)\n',
  PRIMARY KEY (`user_id`, `sequence`),
  INDEX `memory_test_user_idx` (`user_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `memories`.`concentration_test` (
  ...
  `sequence` INT NOT NULL COMMENT '검사 순번\n',
  `measurment_score` INT NOT NULL COMMENT '측정 성공 점수\n',
  `error_count` INT NOT NULL COMMENT '측정 오류 횟수\n',
  `test_date` VARCHAR(16) NOT NULL COMMENT '검사 일자\n',
  `score_gap` INT NULL COMMENT '(직전 순번의 측정된 점수) - (현재 점수의 측정된 점수)',
...
CREATE TABLE IF NOT EXISTS `memories`.`diary` (
  ...
  `write_date` VARCHAR(10) NOT NULL COMMENT '일기 작성 일자 (YYYY-MM-DD)\n',
  ...
  `contents` TEXT NOT NULL COMMENT '일기 내용 (HTML 형식)',
